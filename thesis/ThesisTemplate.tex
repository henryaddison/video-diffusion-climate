...
\begin{align}
      d\mathbf{z}=\left[\mathbf{f}(\mathbf{z}, t)-g(t)^2\mathbf{s}_\theta(\mathbf{z}_t, \lambda_t)\right]dt + g(t)d\bar{\mathbf{w}}
\end{align}
...
We denote the known and unknown elements of a tensor $\mathbf{x}$ by $\mathbf{x}^a$ and $\mathbf{x}^b$, respectively. 
