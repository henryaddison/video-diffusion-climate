...
\end{align}
Song et al. \cite{Score_Based_Song} showed that we can use an It\^{o} stochastic differential equation (SDE) to define the time evolution of the diffusion process:
\begin{align}
      d\mathbf{z}=\mathbf{f}(\mathbf{z},t)dt + g(t)d\mathbf{w}\label{eq:forward_sde}
\end{align}
where $\mathbf{w}$ is the standard Wiener process, $\mathbf{f}(\mathbf{z}, t)$ is a vector-valued function called the \textit{drift} coefficient of $\mathbf{z}$, and $g(t)$ is a scalar function known as the \textit{diffusion} coefficient of $\mathbf{z}$. In this work, we use a \textit{variance-preserving} diffusion model, which is defined by:
\begin{align}
      \mathbf{f}(\mathbf{z},t)&=-\frac{1}{2}\left(\frac{d}{dt}\log\left(1+\exp(-\lambda_t)\right)\right)\mathbf{z}\\
      g(t)^2&=\frac{d}{dt}\log\left(1+\exp(-\lambda_t)\right)
\end{align}
...
Let $\mathbf{s}_\theta(\mathbf{z}_t, \lambda_t)$ be a $\lambda$-dependent score network \cite{Generative_Modelling_By_Estimating_Gradients_Song} that approximates the score of $q(\mathbf{z}_t)$ such that:
\begin{align}
      \mathbf{s}_\theta(\mathbf{z}_t, \lambda_t)\approx \nabla_{\mathbf{z}_t} \log q(\mathbf{z}_t)\label{eq:score_network}
\end{align}
Anderson \cite{Reverse_time_diffusion_Anderson} showed that if we have a perfect score model $\mathbf{s}_\theta(\mathbf{z}_t, \lambda_t)=\nabla_{\mathbf{z}_t}q(\mathbf{z}_t)$ then the forward SDE of Equation \ref{eq:forward_sde} is exactly solved by the reverse-time SDE:
\begin{align}
      d\mathbf{z}=\left[\mathbf{f}(\mathbf{z}, t)-g(t)^2\mathbf{s}_\theta(\mathbf{z}_t, \lambda_t)\right]dt + g(t)d\mathbf{w}
\end{align}
...
\begin{align}
      ...
      &=\mathbb{E}_{\mathbf{z}_0,...\mathbf{z}_1\sim q(\mathbf{z}_0,...,\mathbf{z}_1|\mathbf{x})}\left[-\log\left(\frac{p_\theta(\mathbf{x},\mathbf{z}_0,\ldots,\mathbf{z}_1)}{q(\mathbf{z}_0,\ldots,\mathbf{z}_1|\mathbf{x})}\right)\right]\\
...
\label{sec:background_diffusion_replacement_sampling}
