...
Empirically, we found that performing a square root transformation to the input data significantly improved the quality of the samples generated by our model. Most notably, it significantly reduced the amount of noise at the lower end of the precipitation scale.

\begin{figure}[htbp]
      \centering
      \includegraphics[width=.75\linewidth]{no_transform_vs_sqrt_histogram_low.png}
      \caption{Distribution of precipitation rates below 0.5 millimetres per hour in samples generated via our model trained with the square root transformation compared to both samples generated via our model trained with no transformation and the test set from UKCP18. The horizontal axis is the precipitation rate, measured in millimetres per hour; the vertical axis is frequency density. The vertical axis is scaled logarithmically.}
...
\begin{align}
      ...
      \end{bmatrix}
\end{align}
where we select $\mathrm{P}_{\max}$ as the maximum precipitation rate observed in the training set. In other words, our vector-valued function $\mathbf{h}$ applies an identical transformation $h:[-1, 1]\to[-1,1]$ to each element $x_i$ of $\mathbf{x}$. The change of variable rule states that:
\begin{align}
...
\begin{align}
      \mathcal{L}_{\mathrm{WLCOV}}(\mathbf{x}) = \tilde{\mathcal{L}}_{\mathrm{WL}}(\mathbf{h}(\mathbf{x})) - \sum_{i=1}^D \log \left(\frac{\partial h_\omega(x_i)}{\partial x_i}\right)
\end{align}
...
\subsection{Monotonic Neural Network}

\textbf{THE WEIGHTED LOSS IS DEFINED INCORRECTLY. WE NEED TO DEFINE THE TILDE LWL PROPERLY}
