...
In this work, we employ the $\mathbf{v}$-parameterisation, introduced originally by Salimans and Ho \cite{Progressive_Distillation_Salimans} to facilitate progressive distillation for faster sampling. Though we do not apply progressive distillation in this work, we use the $\mathbf{v}$-parameterisation for the additional benefits highlighted by Ho et al. \cite{Imagen_Video_Ho}: faster convergence of sample quality and prevention of temporal colour shifting sometimes observed with $\boldsymbol\epsilon$-prediction video diffusion models. Formally, the $\mathbf{v}$-parameterisation trains the model to predict:
\begin{align}
...
\begin{align}
      ...
      &=w(\lambda_{\min})\mathcal{L}(\lambda_{\min})+\frac{1}{2}\mathbb{E}_{\lambda\sim p(\lambda)}\left[\frac{w(\lambda)}{p(\lambda)}\Vert\boldsymbol\epsilon-\hat{\boldsymbol\epsilon}(\mathbf{z}_t,\lambda_t)\Vert_2^2\right]
\end{align}
