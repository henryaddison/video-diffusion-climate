...
VAEs are \textit{latent-variable models}. That is, we can think of each observed datapoint $\mathbf{x}\in\mathcal{D}$ as being represented or generated by an associated \textit{latent} variable $\mathbf{z}$. The latent variable is part of the model, but we do not observe it directly---it is not within the dataset. We model the joint distribution of the observed data and latent variable by $p_\theta(\mathbf{x},\mathbf{z})$. 
...
Since VAEs are a likelihood-based generative model, we would ideally train our model to maximise the likelihood of $p_\theta(\mathbf{x}$). However, there is a problem with this approach. Firstly, we cannot simply marginalise out the latent variable $\mathbf{z}$ via:
\begin{align}
