...
One of the primary contributions of this work is demonstrating that a transformation of the input data is critical to the model's performance. To motivate this, we first provide results for a model trained without transforming the input data. Notably, the model produced samples with two significant, related issues. Namely, the generated samples all contain significant perceptible noise at the lower end of the precipitation scale, and many exhibit a baseline shift in precipitation rates. Figure \ref{fig:no_transform_sample} depicts an example of the former, while Figure \ref{fig:no_transform_global_shift} depicts an example of the latter. Generated samples with the aforementioned baseline shift exhibit a modal precipitation rate significantly higher than samples observed in the test set. Visually, this manifests as a consistent blue colour across the entire sample in regions without more intense precipitation, indicating at least a  trace quantity of precipitation across the entire 563.2\ \text{km} $\times$ 563.2\ \text{km} region for the entire ten hours. Even in isolation, this would be an unnatural scenario and thus should certainly not occur at the frequency we observed in the samples generated via our model.
...
As evident from Figure \ref{fig:no_transform_histogram_high}, the distribution of individual cells' values in the samples generated via our model broadly resembles that of our test set from UKCP18. However, the generated samples undeniably underrepresent precipitation events at most intensity levels, particularly those at the extreme end of the scale, as most evident from the quantile-quantile plots in Figure \ref{fig:no_transform_qq}. Most quantiles plotted for the generated samples are approximately 25\% lower than their corresponding quantile in the test set.
...
\begin{figure}[htbp]
      ...
      \end{subfigure}
      \caption{Quantile-quantile plots for individual cells within the test set from UKCP18 against samples generated via our model trained with no transformation. A given point represents the precipitation rate for a given quantile in both the test set and the generated samples. Both plots are for the same samples, with different quantiles plotted; the quantiles plotted are disjoint subsets of $\mathcal{Q} = \{1 - 10^{-n_1} + n_2 \cdot 10^{-n_1 - 1}\mid n_1,n_2\in \mathbb{N}, 1 \le n_2 \le 9\}$.}
      \label{fig:no_transform_qq}
\end{figure}

The underrepresentation of precipitation events beyond trace precipitation, in coalescence with the significant noise at the lower end of the precipitation scale and the baseline shift, induces an overall imbalanced representation of the precipitation distribution. In effect, we can consider the model to be severely overestimating the frequency of trace precipitation in detriment to the frequency of both extreme precipitation and dry---or clear---weather conditions.

\subsection{Square Root Transformation}

Empirically, we found that performing a square root transformation to the input data eliminated the two primary problems observed in samples generated via our model trained with no transformation. Namely, it eliminated any detectable noise at the lower end of the precipitation scale, and none of its samples contained the unnatural baseline shift in precipitation rates. The improvement is evident in Figure \ref{fig:no_transform_vs_sqrt_histogram_low}. Unlike in the case of the model trained without any transformation to the input data, the substantial deviation from the test distribution shape below approximately 0.2 millimetres per hour is absent. We speculate this is due to the square root transformation causing a significant proportion of the space to be occupied by rates at the lower end of the precipitation scale. For example, with no transformation, the region below 0.2 millimetres per hour occupies approximately 0.2\% of the space. Whereas with the square root transformation, the same area occupies approximately 5\% of the space. As such, this equates to approximately a 20-fold increase in the proportion of the space allocated to the same region.
...
Conducting a square root transformation to the input data improved the quality of our samples beyond simply eliminating the two aforementioned problems. As evident from the spatial and temporal PSD graphs in Figure \ref{fig:no_transform_vs_sqrt_psd}, the power in the samples generated via the square root transformation is closer to that of the test set. The improvement holds over both the spatial and temporal dimensions at all scales.
...
Moreover, for less intense precipitation---up to approximately 6 millimetres per hour---quantiles from the model trained via the square root transformation were closer to the ideal quantiles from the test set than those generated via the model trained with no transformation, as evident in Figure \ref{fig:no_transform_vs_sqrt_qq_low}. We can similarly see the superior performance of the model trained via the square root transformation in this region from the distribution in Figure \ref{fig:no_transform_vs_sqrt_histogram_low}. However, for more intense precipitation, the dominance of either model is more ambiguous, as can be seen in Figure \ref{fig:no_transform_vs_sqrt_qq_high}. Namely, the model trained via the square root transformation performed significantly better at the highest end of the scale, above 30 millimetres per hour. However, quantiles from the model trained with no transformation were  closer to the target distribution, particularly in the region from 10--30 millimetres per hour. It is important to note that this is the sole domain wherein we found the square root transformation to yield a minor degradation in sample quality.
...
Suppose we transform our observed variable $\mathbf{x}$ via an element-wise monotonically-increasing vector-valued function $\mathbf{h}:[-1, 1]^D\to[-1,1]^D$, such that:
\begin{align}
      \mathbf{h}(\mathbf{x})=
      \begin{bmatrix}
            h(x_1) \\
      ...
      \end{bmatrix}
\end{align}
In other words, our vector-valued function $\mathbf{h}$ applies an identical transformation $h:[-1, 1]\to[-1,1]$ to each element $x_i$ of $\mathbf{x}$. The change of variable rule states that:
\begin{align}
...
\end{align}
Thus, if we were to parameterise the transformation $\mathbf{h}_\omega$, with transformative parameters $\omega$, we would theoretically be able to learn some optimal transformation to approximately minimise the negative log-likelihood of the observed variable $\mathbf{x}$. We denote 

For completeness, the loss with transformative parameters $\omega$ yields only a minor modification to Equation \ref{eq:optimal_elbo_no_parameters} and is given by:
\begin{align}
...
To address this issue, we introduce two solutions which, in conjunction, we found to yield a significant improvement in the quality of the samples generated by our model. Firstly, we significantly increase the multiple by which we add the input to the transformation network to its output. Consequently, at no point is the gradient of the transformation less than a predetermined value. We thus refer to this small multiple as our minimum gradient coefficient. Empirically, we found that a minimum gradient coefficient of 0.1 yielded good results.

Secondly, we introduce a masking function $m:[-1,1]\to \{0,1\}$ that takes a single element $x_i$ of some datapoint $\mathbf{x}$ as input and outputs a binary value, such that:
\begin{align}
      m(x_i)=\begin{cases}
...
\subsection{Comparison of the Learnt and Square Root Transformations}

Neither our model that learnt its transformation, nor our model trained via the square root transformation, performed flawlessly by all metrics. Both models tended to underrepresent non-zero precipitation events in their generated samples, as evident from Figure \ref{fig:learnt_transform_vs_sqrt_qq}. The output space covered by the quantiles plotted in Figure \ref{fig:learnt_transform_vs_sqrt_qq_low} approximately corresponds to light precipitation---the model that learnt its transformation performs better in this region. However, the dominance of one model over the other is less distinct for the more extreme precipitation events covered by Figure \ref{fig:learnt_transform_vs_sqrt_qq_upper}. The model trained with the square root transformation performs marginally better in two regions---from 5--10 millimetres per hour and above 30 millimetres per hour---though the model that learnt its transformation performs better in the remaining regions.
...
The superior performance of our model that learnt its transformation is most evident from Figure \ref{fig:learnt_transform_vs_sqrt_bias}, which depicts the mean-normalised bias at each cell representing the same geographical area over time and all samples. The mean-normalised bias is the sample mean subtracted by the target mean divided by the target mean. As such, the plots indicate the degree to which the models capture the true amount of precipitation over different geographical areas. 

\begin{figure}[htbp]
      \centering
      \begin{subfigure}{.49\textwidth}
            \includegraphics[width=\linewidth]{sqrt_bias.png}
            \caption{Square Root Transformation}
      \end{subfigure}
      \begin{subfigure}{.49\textwidth}
            \includegraphics[width=\linewidth]{learnt_transform_bias.png}
            \caption{Learnt Transformation}
      \end{subfigure}
      \caption{Mean-normalised bias at each cell representing the same geographical area in the samples generated via our model trained with the square root transformation (left) and our model that learnt its transformation (right). The mean absolute mean-normalised bias is 0.31 for the square root transformation and 0.23 for the learnt transformation.}
      \label{fig:learnt_transform_vs_sqrt_bias}
\end{figure}
