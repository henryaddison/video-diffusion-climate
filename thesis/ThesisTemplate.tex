...
One of the primary contributions of this work is a demonstration that a transformation of the input data is critical to the model's performance. To motivate this, we first provide results for a model trained without transforming the input data. Notably, the model produced samples with significant perceptible noise at the lower end of the precipitation scale. Figure \ref{fig:no_transform_sample} depicts an example of this. While the sample is temporally coherent, the noise is abundantly visible and detrimental to the quality of the sample.
...
To motivate this beyond a single example, Figure \ref{fig:no_transform_histogram_low} depicts the distribution of precipitation rates of our generated samples compared to those in our test set at the lower end of the scale (i.e. less than 0.5 millimetres per hour) where the noise is most perceptible. As can be seen, below approximately 0.2 millimetres per hour, the distribution of precipitation rates in our generated samples significantly deviates from that of the test set. For example, our generated samples contain precipitation rates of 0.1 millimetres per hour ten times more frequently than the test set. While 0.1 millimetres per hour equates to only very light drizzle, it is still a non-negligible amount of precipitation. The fact that the discrepancy is so large at this scale likely means that using the generated samples in any real-world context would present significant challenges.

\begin{figure}[htbp]
      \centering
      \includegraphics[width=.75\linewidth]{no_transform_histogram_low.png}
      \caption{Distribution of precipitation rates below 0.5 millimetres per hour in samples generated via our model trained with no transformation compared to the test set from UKCP18. The horizontal axis is the precipitation rate, measured in millimetres per hour; the vertical axis is frequency density. The vertical axis is scaled logarithmically.}
      \label{fig:no_transform_histogram_low}
\end{figure}

We can attribute much of this deviation to random noise visible in the generated samples. We cannot determine the precise source of this noise, but we speculate it may be a combination of multiple factors. The first contributing factor is likely that our score model $\mathbf{s}_\theta(\mathbf{z}_t, \lambda_t)$ is not perfect; in other words, there likely exists $\mathbf{z}_t$ and $\lambda_t$ such that $\mathbf{s}_\theta(\mathbf{z}_t, \lambda_t)$ does not serve as a good approximation for $\nabla_{\mathbf{z}_t}  \log q(\mathbf{z}_t)$. In addition, since we can only approximate the reverse-time SDE, the discretised numerical solver likely introduces errors itself---this would be the case even if we had a perfect score model. We experimented with predictor-corrector sampling \cite{Score_Based_Song} to rectify this but observed no improvement in the quality of our generated samples. We speculate that the errors introduced by these two factors likely manifest as the noise in our generated samples.

Noise of this magnitude creates unique challenges for diffusion models generating precipitation data---challenges that do not present as significant an issue in the more common domain of image generation. There are two primary reasons for this. First and foremost, the distribution of precipitation rates in our data is far more skewed than the distribution of colours in image data. This skew is evident in Figure \ref{fig:ukcp18_test_histogram}, which depicts the full distribution of precipitation rates in our test set. 80\% of the precipitation rates are less than 0.003 millimetres per hour; to put this into perspective, the highest precipitation rate in our test set is approximately 60 millimetres per hour. As a direct result of this extreme distribution, even relatively small amounts of noise at the lower end of the scale can significantly impact the quality of the generated samples.

\begin{figure}[htbp]
      \centering
      \includegraphics[width=.75\linewidth]{ukcp18_test_histogram.png}
      \caption{Distribution of precipitation rates in the test set from UKCP18. The horizontal axis is the precipitation rate, measured in millimetres per hour; the vertical axis is frequency density. The vertical axis is scaled logarithmically.} 
      \label{fig:ukcp18_test_histogram}
\end{figure}

The second challenge that generating precipitation data presents in contrast to image data is that precipitation is a continuous variable. In contrast, discrete values comprise each colour channel in an image. Thus, a final discretisation step at the end of the generative process for images eliminates much of the noise. We cannot take advantage of this approach for precipitation data. In the context of Figure \ref{fig:no_transform_sample} wherein the noise is most perceptible below the 0.2 millimetres per hour threshold, this constitutes less than 0.2\% of our output space. If this were an image with 256 discrete values per colour channel, this relative amount of noise represents less than a single discrete value. Thus, a final discretisation step would eliminate the noise almost entirely.

One final unnatural characteristic exhibited by a small proportion of the samples generated via our model trained with no transformation of the input data is an unnatural global shift in precipitation rates. Visually, this effect manifests as a consistent blue colour. Specifically, the samples in question exhibit a low precipitation rate in the region of 0.1--0.5 millimetres per hour across the entire sample, which represents a low drizzle rate across the entire 563.2\ \text{km} $\times$ 563.2\ \text{km} region for the entire ten hours. Figure \ref{fig:no_transform_global_shift} depicts one such sample. Even in isolation, this would be an unnatural scenario and thus should certainly not occur at the frequency we observed in the samples we generated. We could not identify a single example in the test set that exhibited this characteristic.

\begin{figure}[htbp]
      \centering
      \includegraphics[width=\linewidth]{no_transform_global_shift.png}
      \caption{Sample generated via our model trained with no transformation of the input data that exhibits an unnatural global shift in precipitation rates. The format is the same as Figure \ref{fig:no_transform_sample}.}
      \label{fig:no_transform_global_shift}
\end{figure}

\subsection{Square Root Transformation}

Empirically, we found that performing a square root transformation to the input data eliminated the two primary problems observed in samples generated via our model trained with no transformation. Namely, the square root transformation eliminated any detectable noise at the lower end of the precipitation scale, and none of its samples contained the unnatural global shift in precipitation rates. The noise elimination at the lower end of the precipitation scale is evident in Figure \ref{fig:no_transform_vs_sqrt_histogram_low}. Unlike in the case of no transformation, the unnatural deviation from the test distribution shape below approximately 0.2 millimetres per hour is absent in the samples generated via the square root transformation.

\begin{figure}[htbp]
      \centering
      \includegraphics[width=.75\linewidth]{no_transform_vs_sqrt_histogram_low.png}
      \caption{Distribution of precipitation rates below 0.5 millimetres per hour in samples generated via our model trained with the square root transformation compared to both samples generated via our model trained with no transformation and the test set from UKCP18. The format is the same as Figure \ref{fig:no_transform_histogram_low}}
      \label{fig:no_transform_vs_sqrt_histogram_low}
\end{figure}

The significant improvement in sample quality is clear evidence that a transformation of the input can be critical to the performance of a diffusion model.
...
Neither our model that learnt its transformation, nor our model trained via the square root transformation, performed flawlessly by all metrics. Both models tended to underrepresent non-zero precipitation events in their generated samples, as evident from Figure \ref{fig:learnt_transform_vs_sqrt_qq}. The output space covered by the quantiles plotted in Figure \ref{fig:learnt_transform_vs_sqrt_qq_lower} approximately corresponds to light precipitation---the model that learnt its transformation performs better in this region. However, the dominance of one model over the other is less distinct for the more extreme precipitation events covered by Figure \ref{fig:learnt_transform_vs_sqrt_qq_upper}. The model trained with the square root transformation performs marginally better in two regions---from 5--10 millimetres per hour and above 30 millimetres per hour---though the model that learnt its transformation performs better in the remaining regions.

\begin{figure}[htbp]
      \centering
      \begin{subfigure}{.495\textwidth}
            \includegraphics[width=\linewidth]{learnt_transform_vs_sqrt_qq_lower.png}
            \caption{Quantiles: $\{q\in\mathcal{Q}\mid 0.91 \le q \le 0.99\}$}
            \label{fig:learnt_transform_vs_sqrt_qq_lower}
      \end{subfigure}
      \begin{subfigure}{.485\textwidth}
            \includegraphics[width=\linewidth]{learnt_transform_vs_sqrt_qq.png}
            \caption{Quantiles: $\{q\in\mathcal{Q}\mid 0.991 \le q \le 0.9999999 \}$}
            \label{fig:learnt_transform_vs_sqrt_qq_upper}
      \end{subfigure}
      \caption{Quantile-quantile plots for individual cells within the test set from UKCP18 against both samples generated via our model trained with the square root transformation and samples generated via our model that learnt its transformation. A given point represents the precipitation rate for a given quantile in both the test set and the generated samples. Green points are for the square root transformation; red points are for the learnt transformation. Both plots are for the same samples, with different quantiles plotted; the quantiles plotted are disjoint subsets of $\mathcal{Q} = \{1 - 10^{-n_1} + n_2 \cdot 10^{-n_1 - 1}\mid n_1,n_2\in \mathbb{N}, 1 \le n_2 \le 9\}$.}
      \label{fig:learnt_transform_vs_sqrt_qq}
\end{figure}

\begin{figure}[htbp]
      \centering
      \includegraphics[width=.75\linewidth]{learnt_transform_vs_sqrt_histogram.png}
      \caption{Distribution of precipitation rates in samples generated via our model trained with the square root transformation, our model that learnt its transformation, and the test set from UKCP18. The format is the same as Figure \ref{fig:ukcp18_test_histogram}.}
...
\begin{figure}[htbp]
      ...
      \end{subfigure}
      \caption{Power spectral density (PSD) plots for samples generated via our model trained with the square root transformation, our model that learnt its transformation, and the test set from UKCP18. The horizontal axis is the number of waves, denoted $k$; the vertical axis is the mean power across all samples for a given $k$, denoted $P(k)$. Both axes are logarithmic. The left plot is a PSD plot for two spatial dimensions, and the right plot is a PSD plot for the temporal dimension.}
